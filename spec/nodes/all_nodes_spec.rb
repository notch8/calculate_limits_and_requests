...
RSpec.describe Nodes::AllNodes do
  ...
  describe '#current_pod_counts' do
    it 'returns an array of hashes' do
      expect(described_class.current_pod_counts).to be_an_instance_of(Array)
      expect(described_class.current_pod_counts.size).to eq(7)
      expect(described_class.current_pod_counts.first.keys).to match_array(%i[node pod_count])
    end
  end

  describe '#container_resources' do
    it 'returns an array of hashes' do
      resources = described_class.container_resources
  ...
  describe '#sum_of_resources_by_node' do
    it 'returns an array of hashes - one per node' do
      resources = described_class.sum_of_resources_by_node
      expect(resources).to be_an_instance_of(Array)
