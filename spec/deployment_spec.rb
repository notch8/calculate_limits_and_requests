...
RSpec.describe Deployment do
  ...
  let(:deployment) do
    described_class.new(namespace: 'palni-palci-knapsack', owner_name: 'hyku', container_name: 'hyku')
  end
  ...
  let(:redis_deployment) do
    described_class.new(namespace: 'palni-palci-knapsack', owner_name: 'hyku', container_name: 'redis')
  end
  ...
  describe '#initialize' do
    it 'creates a deployment with namespace, owner_name, and container_name' do
      expect(deployment.namespace).to eq('palni-palci-knapsack')
      expect(deployment.owner_name).to eq('hyku')
      expect(deployment.container_name).to eq('hyku')
  ...
  describe '#container_type' do
    context 'with hyku containers' do
      before do
        deployment.add_container(container1)
        deployment.add_container(container2)
      end
      ...
      it 'returns the container type' do
        expect(deployment.container_type).to eq(:rails_app)
      end
    end

    context 'with redis containers' do
      before do
        redis_deployment.add_container(redis_container)
      end

      it 'returns the container type' do
        expect(redis_deployment.container_type).to eq(:cache)
      end
    end

    context 'with no containers' do
      it 'returns utility as default' do
        expect(deployment.container_type).to eq(:utility)
      end
    end
  end

  describe '#pod_count' do
    it 'returns the number of container instances (pods)' do
      deployment.add_container(container1)
      deployment.add_container(container2)
      deployment.add_container(container3)
      expect(deployment.pod_count).to eq(3)
    end
  end
  ...
  describe 'max value calculations' do
    ...
    describe '#max_cpu_request_current' do
      it 'returns the maximum CPU request across all container instances' do
    ...
    describe '#max_cpu_limit_current' do
      it 'returns the maximum CPU limit across all container instances' do
    ...
    describe '#max_memory_request_current' do
      it 'returns the maximum memory request across all container instances' do
    ...
    describe '#max_memory_limit_current' do
      it 'returns the maximum memory limit across all container instances' do
    ...
    describe '#max_cpu_95' do
      it 'returns the maximum 95th percentile CPU across all container instances' do
    ...
    describe '#max_cpu_99' do
      it 'returns the maximum 99th percentile CPU across all container instances' do
    ...
    describe '#max_memory_95' do
      it 'returns the maximum 95th percentile memory across all container instances' do
    ...
    describe '#max_memory_99' do
      it 'returns the maximum 99th percentile memory across all container instances' do
    ...
    describe '#max_memory_max' do
      it 'returns the maximum memory usage across all container instances' do
  ...
  describe '#stanza' do
    ...
    it 'generates a YAML stanza for container configuration' do
  ...
  describe '.headers' do
    it 'returns an array of column headers' do
      ...
      expect(headers).to include('namespace', 'owner', 'container', 'container_type', 'pod_count')
  ...
  describe '#row' do
    ...
    it 'returns an array with all deployment data' do
      ...
      expect(row.length).to eq(19) # All columns including stanza
    end

    it 'includes namespace, owner_name, and container_name' do
      row = deployment.row
      expect(row[0]).to eq('palni-palci-knapsack')
      expect(row[1]).to eq('hyku')
      expect(row[2]).to eq('hyku')
    end

    it 'includes container type and pod count' do
      row = deployment.row
      expect(row[3]).to eq(:rails_app) # container_type
      expect(row[4]).to eq(3) # pod_count
    end

    it 'includes maximum current values' do
      row = deployment.row
      expect(row[5]).to eq(200) # max_cpu_request_current
      expect(row[6]).to eq(2000) # max_cpu_limit_current
      expect(row[7]).to eq(2048) # max_memory_request_current
      expect(row[8]).to eq(4096) # max_memory_limit_current
    end

    it 'includes quantile values' do
      row = deployment.row
      expect(row[9]).to be_an(Integer) # cpu_95
      expect(row[10]).to be_an(Integer) # cpu_99
      expect(row[11]).to be_an(Integer) # memory_95
      expect(row[12]).to be_an(Integer) # memory_99
      expect(row[13]).to be_an(Integer) # memory_max
    end

    it 'includes recommended values' do
      row = deployment.row
      expect(row[14]).to be_an(Integer) # cpu_request_recommended
      expect(row[15]).to be_an(Integer) # cpu_limit_recommended
      expect(row[16]).to be_an(Integer) # memory_request_recommended
      expect(row[17]).to be_an(Integer) # memory_limit_recommended
    end

    it 'includes stanza as last element' do
      row = deployment.row
      expect(row[18]).to be_a(String)
      expect(row[18]).to include('resources:')
    end
  end
